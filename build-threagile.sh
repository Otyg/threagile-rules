...
go build -a -trimpath -ldflags="-s -w -X main.buildTimestamp=$(date '+%Y%m%d%H%M%S')" -gcflags="all=-trimpath=/src" -asmflags="all=-trimpath=/src" -buildmode=plugin -o use-of-weak-cryptography.so custom/use-of-weak-cryptography/use-of-weak-cryptography.go